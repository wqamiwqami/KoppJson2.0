...
static std::vector<std::string>KOPP_TYPE_STR = {
    "KOPP_NULL" , "KOPP_FALSE" , "KOPP_TRUE" , 
    "KOPP_NUMBER" , "KOPP_STRING" , "KOPP_ARRAY" , "KOPP_OBJECT"
};

//错误码
enum {
    KOPP_NO_ERROR                           = 0,   //默认, 无错误
...
//使用静态类，完成关键功能
// 字符串、文件转换为Json ， Json转换为文件、字符串
// 有异常会throw出错误
class Json {
friend class KoppJson;
public:
    static KoppValue*   StrToJson    (const std::string& str);
    static std::string  JsonToStr    (KoppValue& val , bool hasLevel = false);
    static KoppValue*   ReadJsonFile (const std::string& path);
    static void         SaveJsonFile (KoppValue& val , const std::string& path , bool hasLevel = false);
};

// json值
// 这里没有使用继承的方法，这是一个独立的类
// 用一个void指针指向对象，一个type指向类型
class KoppValue{
public:
    KoppValue();
    ~KoppValue();
    ...
    KoppValue(const KoppValue&   val);

    std::string get_type(){return KOPP_TYPE_STR[this->type];};
    bool        is_null (){return (this->type == KOPP_NULL);};

    KoppValue   copy      ( const  KoppValue& val ){return KoppValue(val);};
    KoppValue*  copy_from ( const  KoppValue& val );//复制另一个的信息
    
    bool clear   ( );//清除原有信息
    
    void setLevel( int level );//setLevel递归调用,让下属所有元素都set
    ...
    kopp_type type;  //值类型
    int       level; //深度,上层每有一个object或者array,深度+1,顶层为0
};


//json,值和名称组成的数对,object的组成  "Key":"Val" 键值对
class KoppPair{
public:
...
public:
    ...
    void push_back(KoppPair* pair){pair_list.push_back(std::move(pair));};
...
// 基本已废弃 , 可以利用这个指针做一些特殊操作，比如简单地回到父级目录
class KoppJsonPtr{
    //构造了一个反向链表,便于回溯
    class node{
    public:
...
friend class Json;
public:
    KoppJson(){this->VAL = new KoppValue();JP.set_root(this->VAL);};
    ~KoppJson(){};
    ...
    int KoppParseArray (void* &res , int& level);
    int KoppParseObject(void* &res , int& level);

    //JP相关,调用当前位置
    KoppValue*  ptr()     {return JP.get_ptr(); };//获取位置指针
    std::string get_path(){return JP.get_path();};//获取路径(字符串形式)

    KoppValue get_VAL(){return (*VAL);};
    KoppValue* get_VAL_p(){return VAL;}
