...
static int KoppParseLiteral(kopp_type& type , std::string::iterator& ch , const char* literal ,  void* &res);
static int KoppParseString(std::string::iterator& ch ,  void* &res);
static void KoppParseWhitespace(std::string::iterator& ch);
static int KoppParseNumber(std::string::iterator& ch ,  void* &res);
...
static int creatNo = 0;
void creatInfo(kopp_type type , void* in , bool show){
    creatNo++;
    if(show) std::cout<<"new No:"<<(creatNo)<<" type:"<<type<<" "<<in<<'\n';
}
...
int KoppParseLiteral(kopp_type& type , std::string::iterator& ch , const char* literal ,  void* &res){
    ...
    else{res = new bool(type == KOPP_TRUE);
#ifdef SHOWMEM 
    creatInfo(type,res);
#endif
...
int KoppParseNumber(std::string::iterator& ch ,  void* &res){
    ...
    // 使用范围构造函数
    std::string num(ch, p+1);
    ch = p;

    try{
        (res) = new double(stod(num, NULL));
...
int KoppJson::KoppParseArray( void* &res , int& level){
...
int KoppJson::KoppParseObject( void* &res, int& level){
...
int KoppJson::build(KoppValue* &val){
    ...
    switch (*ch){
        ...
        case '[' : { val->type = KOPP_ARRAY;  return KoppParseArray   (                           val->vp , val->level); }
        case '{' : { val->type = KOPP_OBJECT; return KoppParseObject  (                           val->vp , val->level); }
...
#ifdef SHOWMEM 
    std::cout<<"sum new :"<<creatNo<<std::endl;
#endif
...
std::string KoppJson::get_error_str(){
    std::string::iterator it1 = (ch - str.begin() < 15)?str.begin():ch -15;
    std::string::iterator it2 = (str.end() - ch < 15) ?str.end() : ch+15;
