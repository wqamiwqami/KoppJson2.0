...
namespace Kopp{
//复制构造
KoppValue::KoppValue(const KoppValue& val){
    this->type = val.type;
...
KoppPair::KoppPair(KoppPair& pair){
    ...
    this->key_str = (std::string*)(this->key->vp);
}
...
//复制另一个的信息,注意会先进行清除操作
KoppValue* KoppValue::copy_from (const KoppValue& val){
    this->clear();
...
int KoppValue::makePair(std::string key , std::string value , int& error){
    ...
    KoppValue* val_p = new KoppValue(json.get_VAL());
